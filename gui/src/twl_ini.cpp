...
public:
	...
	void remove_section();
	void remove_key(pchar key);
	const gui_string get_keys(pchar key);
	const gui_string get_sections();
	const gui_string read_string(pchar key, pchar def = L"");
	double read_number(pchar key, double def = 0.0);
	const TCHAR* get_path();
	const TCHAR* get_section();
};
...
const gui_string IniFile::read_string(pchar key, pchar def)
{
	const size_t buffsize = 1024;
	gui_string tmp(buffsize, 0);
...
const TCHAR* IniFile::get_path()
{
	return m_file;
}

const TCHAR* IniFile::get_section()
{
	return m_section;
}
...
int do_remove_section(lua_State* L)
{
	...
	//pini->read_number(StringFromUTF8(key).c_str(), def);
	return 0;
}
