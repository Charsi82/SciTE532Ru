...
std::unique_ptr<TWin> get_desktop_window()
{
	return std::make_unique<TWin>(GetDesktopWindow());
}
...
int tabbar_add(lua_State* L)
{
	...
	{
		tab->add(StringFromUTF8(luaL_checkstring(L, 2)).data(), window_arg(L, 3), luaL_optinteger(L, 4, -1));
	}
	return 0;
}
...
int new_groupbox(lua_State* L)
{
	if (TWin* p = get_last_parent()) {
		auto caption = luaL_checkstring(L, 1);
		return wrap_window(L, new TGroupBoxLua(p, StringFromUTF8(caption).c_str(), L));
	}
	else {
...
{
	TButtonBase* btn = (TButtonBase*)window_arg(L);
	if (!btn) return 0;
...
int window_client(lua_State* L)
{
	...
	if (TWin* child = window_arg(L, 2))
	{
		...
		throw_error(L, "must provide a child window");
	return 0;
}
...
{
public:
	TTreeViewLua(TEventWindow* parent, lua_State* L, bool has_lines = true)
